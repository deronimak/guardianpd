...
5. **Real-time notification to *all* guardians** linked to that child, not just the scanning one — turns every other guardian into a passive fraud detector. Implemented (`app/core/push.py` + fan-out in `app/api/routes/attendance.py`) via Firebase Cloud Messaging, with a log-only fallback when no Firebase project is configured. The fan-out is best-effort: one guardian's dead/invalid device token is pruned automatically and can't fail the scan or block anyone else's notification (a real gap caught by testing against a live Firebase project). The Flutter app registers real device tokens on Android (`lib/core/push_registration.dart`) using just `google-services.json` — deliberately avoiding the Firebase CLI / `flutterfire configure`, which would need your Google account login. iOS isn't wired up — and unlike everything else on the "not yet built" list, this one's a hard environment blocker, not just undone work: it needs Xcode on macOS, which this Windows-based build has no path to. See README "Push notifications" for the one file you still need to add on Android.
